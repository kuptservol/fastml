...
def notebook2script(fname=None, fname_out=None, allFiles=None, upTo=None):
    ...
    if not allFiles:
        notebook2scriptSingle(fname, fname_out)
    else:
        print('Begin...')
...
def notebook2scriptSingle(fname, fname_out=None):
    "Finds cells starting with `#export` and puts them into a new module"
    fname = Path(fname)
    if not fname_out:
